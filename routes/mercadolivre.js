...
// Nova rota para buscar anuncios do usuario autenticado
router.get("/items", async (req, res) => {
  const userId = "default_user"; // Para futuro: usar ID do usuario logado
  const marketplace = "mercadolivre";

  try {
    // Obter token valido usando a funcao auxiliar existente
    const accessToken = await getValidAccessToken(userId, marketplace);
    
    // Buscar informacoes do usuario para obter o seller_id
    const userInfoResponse = await axios.get("https://api.mercadolibre.com/users/me", {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    ...
    res.status(500).json({ 
      message: "Erro ao buscar anuncios", 
      error: error.message,
