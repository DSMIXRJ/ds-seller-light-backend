...
console.log("[INDEX_LOG] Starting DS Seller Backend with Supabase PostgreSQL...");
...
app.get("/", (req, res) => {
  ...
  res.send("DS Seller Backend with Supabase PostgreSQL is running! Check logs for DB status.");
});

app.listen(port, () => {
  console.log(`[INDEX_LOG] Server is running on port ${port}.`);
  console.log("[INDEX_LOG] Supabase PostgreSQL connection attempt is handled by database.js on load.");
